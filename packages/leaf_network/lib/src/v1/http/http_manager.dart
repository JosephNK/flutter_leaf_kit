...
class HTTPManager extends http.BaseClient {
  ...
  Future<http.Response> head(
    ...
    try {
      ...
      final response = await _sendUnstreamed('HEAD', url, httpHeaders).timeout(
          timeout,
          onTimeout: () => throw LeafTimeoutRequestException(timeOutMessage));
  ...
  Future<http.Response> post(
    ...
    try {
      ...
      final httpHeaders = await _getHttpHeaders(uri: uri, headers: headers);
      final response =
          await _sendUnstreamed('POST', url, httpHeaders, body, encoding)
              .timeout(timeout,
                  onTimeout: () =>
                      throw LeafTimeoutRequestException(timeOutMessage));
      final statusCode = response.statusCode;
  ...
  Future<http.Response> put(
    ...
    try {
      ...
      final httpHeaders = await _getHttpHeaders(uri: uri, headers: headers);
      final response =
          await _sendUnstreamed('PUT', url, httpHeaders, body, encoding)
              .timeout(timeout,
                  onTimeout: () =>
                      throw LeafTimeoutRequestException(timeOutMessage));
      final statusCode = response.statusCode;
  ...
  Future<http.Response> patch(
    ...
    try {
      ...
      final httpHeaders = await _getHttpHeaders(uri: uri, headers: headers);
      final response =
          await _sendUnstreamed('PATCH', url, httpHeaders, body, encoding)
              .timeout(timeout,
                  onTimeout: () =>
                      throw LeafTimeoutRequestException(timeOutMessage));
      final statusCode = response.statusCode;
  ...
  Exception _throwException(int statusCode, String body) {
    if (statusCode == 400) {
      throw LeafBadRequestException(body);
    } else if (statusCode == 401) {
      throw LeafUnauthorisedException(body);
    } else if (statusCode == 404) {
      throw LeafNotFoundException(body);
    } else if (statusCode == 408) {
      throw LeafTimeoutRequestException(body);
    } else {
      throw LeafServerInternalException(body);
    }
  }
  ...
  Future<Exception?> _throwConnectivityResultException() async {
    ...
    if (connectivityResult == ConnectivityResult.none) {
      throw LeafInternetNotConnectRequestException(internetMessage);
    }
    return null;
  }
