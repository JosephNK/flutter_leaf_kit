...
import '../cache/lf_image_lru_cache_v2.dart';

/// Callback fired when the selection changes.
typedef LFPhotoSelectionChangedV2 = void Function(
  List<AssetEntity> selectedEntities,
);

/// Callback fired when the selection limit is exceeded.
typedef LFPhotoLimitErrorV2 = void Function(Exception error, int limit);
...
/// Uses [LFImageLruCacheV2] for thumbnail caching.
class LFPhotoListViewV2 extends StatefulWidget {
  ...
  final LFPhotoSelectionChangedV2? onSelectionChanged;
  final LFPhotoLimitErrorV2? onLimitError;

  const LFPhotoListViewV2({
    super.key,
  ...
  @override
  State<LFPhotoListViewV2> createState() => _LFPhotoListViewV2State();
}

class _LFPhotoListViewV2State extends State<LFPhotoListViewV2> {
  ...
  @override
  void didUpdateWidget(covariant LFPhotoListViewV2 oldWidget) {
...
class _PhotoThumbnailV2 extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final cached = LFImageLruCacheV2.getData(entity, _cacheSize);
    if (cached != null) {
    ...
    return FutureBuilder<Uint8List?>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.done &&
            snapshot.data != null) {
          LFImageLruCacheV2.setData(entity, _cacheSize, snapshot.data!);
