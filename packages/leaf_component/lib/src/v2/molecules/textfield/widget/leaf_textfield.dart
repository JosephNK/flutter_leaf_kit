...
import '../controller/leaf_textfield_controller.dart';

/// A themed text field widget that resolves all colors from the LF design
/// token system instead of hardcoded values.
///
/// Uses [LeafTextFieldController] for programmatic control.
///
/// Style resolution order:
///   1. Explicit widget parameters
///   2. [LeafTextFieldThemeData] from the nearest [LeafTheme]
///   3. Global tokens ([LeafColors])
@immutable
class LeafTextField extends StatefulWidget {
  ...
  @override
  State<LeafTextField> createState() => _LeafTextFieldState();
}

class _LeafTextFieldState extends State<LeafTextField> {
  ...
  void _onControllerNotify() {
    ...
    switch (controller.status) {
      case LeafTextFieldStatus.reset:
        _reset();
      case LeafTextFieldStatus.clear:
        _clear();
      case LeafTextFieldStatus.setText:
        text = controller.value ?? '';
      case LeafTextFieldStatus.none:
