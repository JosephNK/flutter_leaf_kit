...
import 'package:flutter_leaf_component/src/v2/organisms/photo/cache/leaf_image_lru_cache.dart';
...
void main() {
  group('LeafImageLruCache', () {
    ...
    test('getData returns null for empty cache', () {
      ...
      expect(LeafImageLruCache.getData(entity), isNull);
    });
    ...
    test('setData and getData round-trip', () {
      ...
      LeafImageLruCache.setData(entity, 64, data);

      final result = LeafImageLruCache.getData(entity, 64);
    ...
    test('different sizes are stored separately', () {
      ...
      LeafImageLruCache.setData(entity, 64, data64);
      LeafImageLruCache.setData(entity, 128, data128);

      expect(LeafImageLruCache.getData(entity, 64), equals(data64));
      expect(LeafImageLruCache.getData(entity, 128), equals(data128));
    });

    test('clearCache removes all entries', () {
      final entity = _makeAsset('test-4');
      LeafImageLruCache.setData(entity, 64, Uint8List.fromList([1]));

      expect(LeafImageLruCache.getData(entity, 64), isNotNull);

      LeafImageLruCache.clearCache();

      expect(LeafImageLruCache.getData(entity, 64), isNull);
    });
    ...
    test('same entity+size overwrites without increasing length', () {
      ...
      LeafImageLruCache.setData(entity, 64, Uint8List.fromList([1]));
      LeafImageLruCache.setData(entity, 64, Uint8List.fromList([2, 3]));

      expect(LeafImageLruCache.length, 1);
      expect(
        LeafImageLruCache.getData(entity, 64),
