...
class _TestScreen extends LFScreenStatefulWidgetV2 {
  final String bodyText;
  final bool useCustomScreen;
  final Color? overrideBg;
  final bool overrideUseSafeArea;
  final SafeAreaInsetsV2? overrideSafeAreaInsets;
...
class _TestScreenState extends LFScreenStateV2<_TestScreen> {
  @override
  bool get useSafeArea => widget.overrideUseSafeArea;

  @override
  SafeAreaInsetsV2 get safeAreaInsets =>
      widget.overrideSafeAreaInsets ?? const SafeAreaInsetsV2.all();
...
class _MinimalScreen extends LFScreenStatefulWidgetV2 {
...
class _MinimalScreenState extends LFScreenStateV2<_MinimalScreen> {
  @override
...
void main() {
  group('LFScreenStatefulWidgetV2', () {
  ...
  group('SafeAreaInsetsV2', () {
    test('all() enables every edge', () {
      const insets = SafeAreaInsetsV2.all();
    ...
    test('none() disables every edge', () {
      const insets = SafeAreaInsetsV2.none();
    ...
    test('only() enables selected edges', () {
      const insets = SafeAreaInsetsV2.only(top: true, bottom: true);
      expect(insets.left, isFalse);
      expect(insets.top, isTrue);
      expect(insets.right, isFalse);
      expect(insets.bottom, isTrue);
    ...
    test('fromLTRB sets edges in order', () {
      const insets = SafeAreaInsetsV2.fromLTRB(true, false, true, false);
      expect(insets.left, isTrue);
      expect(insets.top, isFalse);
      expect(insets.right, isTrue);
      expect(insets.bottom, isFalse);
  ...
  group('LFScreenStatelessWidgetV2', () {
    testWidgets('renders child unchanged', (tester) async {
      await tester.pumpWidget(
        wrapWithTheme(
          const LFScreenStatelessWidgetV2(child: Text('Stateless')),
        ),
      );
